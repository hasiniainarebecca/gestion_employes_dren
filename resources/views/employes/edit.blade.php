	                ...
	                <div class="col-12 col-md-8">
		                <div class="app-card app-card-settings shadow-sm p-4">
						    ...
						    <div class="app-card-body">
							    <form class="settings-form" method="POST" action="{{ route('employe.update',$employe->id) }}" enctype="multipart/form-data">
                                    @csrf
                                    @method('PUT')
...
<div class="mb-3">
									    <label for="setting-input-7" class="form-label">IM</label>
									    <input type="number" class="form-control" id="setting-input-7" placeholder="Entrer IM" value="{{ $employe->montant_journalier }}" name="montant_journalier">
  ...
  <circle cx="8" cy="4.5" r="1"/>
									    ...
									    <input type="text" class="form-control" id="setting-input-1" placeholder="Entrer le nom" name="nom" value="{{ $employe->nom }}" required>
										@error('nom')
											<div class="text-danger">{{ $message }}</div>
									    @enderror
									</div>
									<div class="mb-3">
									    ...
									    <input type="text" class="form-control" id="setting-input-2" placeholder="Entrer le prénom" name="prenom" value="{{ $employe->prenom }}" required>
										@error('prenom')
											<div class="text-danger">{{ $message }}</div>
									    @enderror
									</div>
								    <!--<div class="mb-3">
									    <label for="setting-input-3" class="form-label">Date de naissance</label>
									    <input type="date" class="form-control" id="setting-input-3" placeholder="Entrer la date de naissance" value="{{ $employe->date_naissance }}" name="date_naissance">
                                    ...
                                    <div class="mb-3">
									    ...
									    <select name="sexe" class="form-control" id="setting-input-4" value="{{ $employe->sexe }}">
                                    ...
                                    <div class="mb-3">
									    ...
									    <input type="email" class="form-control" id="setting-input-5" placeholder="nom@gmail.com" value="{{ $employe->email }}" name="email">
										@error('email')
											<div class="text-danger">{{ $message }}</div>
									    @enderror
									</div>
                                    <div class="mb-3">
									    ...
									    <input type="text" class="form-control" id="setting-input-6" placeholder="Entrer le numéro de télephone" value="{{ $employe->contact }}" name="contact">
										@error('conatct')
											<div class="text-danger">{{ $message }}</div>
									    @enderror
									</div>
                                    ...
                                    <div class="mb-3">
									    <label for="setting-input-8" class="form-label">Service</label>
                                    ...
                                    <div class="mb-3">
									    <label for="setting-input-9" class="form-label">Photo d'identité</label>
									    <input type="file" class="form-control" id="photo" name="photo" accept="image/*" placeholder="Ajouter une photo d'identité">
										@error('photo')
											<div class="text-danger">{{ $message }}</div>
									    @enderror
										@if($employe->photo)
            								<img src="{{ asset('storage/' . $employe->photo) }}" alt="Photo de {{ $employe->nom }}" class="img-thumbnail mt-2" width="100">
       									 @endif
									</div>
									<button type="submit" class="btn app-btn-primary" >Mettre à jour</button>
									<a href="{{ route('employe.index') }}" class="btn btn-primary" style="color:white;">Annuler</a>
							    </form>
						    </div><!--//app-card-body-->
